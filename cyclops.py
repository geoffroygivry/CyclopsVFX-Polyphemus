...
def assets_view():
    ...
    return render_template("assets.html", side_lib=side_lib, tagz=tagz, assets=assets, user_session=user_session, assets_view=assets_view)

@app.route('/libraries/')
def libraries():
    if 'username' in session:
        user_session = mongo.db.users.find_one({"name": session['username']})
        asset_details = mongo.db.assets.find()
        assets = [x for x in mongo.db.assets.find()]
        side_lib = mongo.db.libraries.find()
        tages = "RGBA-model_house/3d-walls*details#wall$piece mounted whitespace"
        tagz = re.split("[, \-!?_:/.*#$%&]+", tages)

    return render_template("libraries.html", side_lib=side_lib, tagz=tagz, assets=assets, user_session=user_session, assets_view=assets_view)

@app.route('/libraries-view/<lib_name>')
def get_assets(lib_name):
    get_lib = mongo.db.libraries.find_one({"lib_name": lib_name})
    get_assets_from_lib = get_lib.get("assets")
    print (get_assets_from_lib)
    for asset in get_assets_from_lib:
        print(asset)
        det = mongo.db.assets.find({'_id': ObjectId(str(asset))})
...
def libraries_view(lib_name):
    if 'username' in session:
        ...
        side_lib = mongo.db.libraries.find()
        selected_lib = mongo.db.users.find_one({"lib_name": lib_name})
        tages = "RGBA-model_house/3d-walls*details#wall$piece mounted whitespace"
        tagz = re.split("[, \-!?_:/.*#$%&]+", tages)

    return render_template("libraries.html", asset_details=asset_details, side_lib=side_lib, tagz=tagz, assets=assets, user_session=user_session, assets_view=assets_view)


@app.route('/libraries/create/')
def libraries_create():
    return render_template()

def create_library(name, creator, shows, comment, type, assets, tags):
    ...
    db.libraries.insert(
        {
            ...
            "lib_created" : ISODate("2017-06-06T14:16:29.639Z"),
            "lib_changed" : ISODate("2017-06-06T14:16:29.639Z"),
            "Show" : shows,
            "timestamp" : ISODate("2017-06-06T14:16:29.639Z"),
            "comment" : comment,
            "thumbnail_s3" : "https://s3.amazonaws.com/cyclopsvfx/.MANOR_010_v001_comp.thumbnail.jpg",
            "type" : type,
            "lib_uid" : "59aa770558ca3656974cf189",
            "assets" : [ 
                "aseetid1", 
                "assetid2", 
                "assetid3"
