...
def banner_and_thumb(filename, shot_name):
    ...
    if os.path.isfile(os.path.join(tmp_folder, filename)):
        ...
        subprocess.check_call("convert -resize 700 %s/%s %s/%s" % (tmp_folder, filename, tmp_folder, shot_banner), shell=True)
        subprocess.check_call("convert -resize 290 %s/%s %s/%s" % (tmp_folder, shot_banner, tmp_folder, shot_thumb), shell=True)
